...
(ns pdf-stamper.manual.transform-rotate
  (:require
    [pdf-stamper :refer :all]
    [clojure.edn :as edn]))

(def template-1 (edn/read-string (slurp "test/templates/transform_rotate/template-1.edn")))
(def template-pdf-1 "test/templates/transform_rotate/template-1.pdf")

(def background (javax.imageio.ImageIO/read (clojure.java.io/as-file "test/templates/transform_rotate/background.png")))
(def text "Ålquiver")
...
(.writeTo out (java.io.FileOutputStream. "out/transform-rotate.pdf"))
(.close out)
