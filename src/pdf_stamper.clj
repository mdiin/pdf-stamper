...
;; The values available out of the box for the `:type` key are described next.
;;
;; ### Image holes
;;
;; Holes where `:type` is `:image`. In addition to the above keys image holes must have an `:aspect` key. The image is always shrunk to fit the
;; dimensions of the hole; the value of this key defines whether the image is shrunk to fit, or aspect ratio is preserved. Values are `:fit` or `:preserve`,
;; with `:preserve` being the default.
